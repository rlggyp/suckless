...
static Key keys[] = {
	...
	{ 0,														XK_Print,	                			spawn,					SHCMD("flameshot full --path ~/Pictures/screenshot_$(date +%Y-%m-%d_%H-%M-%S).png")},
	{ MODKEY,												XK_Print,	              			  spawn,					SHCMD("flameshot gui --path ~/Pictures/screenshot_$(date +%Y-%m-%d_%H-%M-%S).png")},
	{ MODKEY|ShiftMask,							XK_Print,				                spawn,					SHCMD("flameshot full --clipboard")},
